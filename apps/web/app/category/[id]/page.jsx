...
const C = {
  ...
  grid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(190px, 1fr))', gap: 10, marginBottom: 24 },
  card: {
    backgroundColor: '#0a1f14', border: '1px solid #1a3a2a',
    borderRadius: 12, padding: '14px 12px', cursor: 'pointer', textAlign: 'right',
  },
  cardName: { color: '#e8f5e9', fontSize: 14, fontWeight: 600, marginBottom: 2 },
  ...
  badge: { color: '#4a7c59', fontSize: 11, backgroundColor: '#1a3a2a', padding: '2px 8px', borderRadius: 10 },
  ...
  return (
    <div style={C.page}>
      ...
      {subcategories.length > 0 && (
        <>
          <div style={C.sectionTitle}>תת-קטגוריות</div>
          <div style={C.grid}>
            {subcategories.map((sub) => (
              ...
              >
                ...
                {sub.child_count > 0 && <div style={C.cardSub}>{sub.child_count} תת-קטגוריות</div>}
              </div>
            ))}
          </div>
        </>
      ...
      {lessons.length > 0 && (
        <>
          <div style={{ ...C.sectionTitle, marginTop: 16 }}>שיעורים</div>
          <div style={C.list}>
            {lessons.map((l, idx) => (
              <LessonRow
                key={l.id}
                lesson={l}
                idx={idx}
                onPlay={() => l.link ? playLesson(l, lessons.filter(x => x.link)) : router.push(`/lesson/${l.id}`)}
                onNavigate={() => router.push(`/lesson/${l.id}`)}
              />
            ))}
          </div>
        </>
      ...
      {!isLoading && series.length === 0 && lessons.length === 0 && (
        <div style={C.empty}>לא נמצאו תוצאות</div>
      )}
    </div>
  );
}

function LessonRow({ lesson, idx, onPlay, onNavigate }) {
  return (
    <div
      style={C.row}
      onClick={onPlay}
    ...
    >
      ...
      {lesson.has_audio && <span style={{ fontSize: 18 }}>🎵</span>}
      <span
